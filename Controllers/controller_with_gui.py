...
class Window(Frame):
    ...
    def __init__(self, master):
        ...
        self._draw_robot(START, self._robot.facing)
    ...
    def _init_window(self):
        ...
        #
        # self._explore_entry = Label(bg_frame, text=str(self._explore_limit), justify='center')
        # self._explore_entry.grid(row=0, column=1)
        #
        self._percentage_completion_label = Label(bg_frame, text="0.0%")
        self._percentage_completion_label.grid(row=0, column=0)
        #
        # self._time_limit_label = Label(bg_frame, text="Time Limit(seconds):")
        # self._time_limit_label.grid(row=1, column=0)
        #
        # self._time_limit_entry = Label(bg_frame, text=str(self._time_limit), justify='center')
        # self._time_limit_entry.grid(row=1, column=1)
        #
        self._time_spent_label = Label(bg_frame, text="0.0s")
        self._time_spent_label.grid(row=1, column=0)
    ...
    def _turn_head(self, facing, direction):
        ...
        if facing == NORTH:
            if direction == LEFT:
                ...
                self._robot.facing = WEST
            elif direction == RIGHT:
                self._canvas.move(self._head, self._grid_size, self._grid_size)
                self._robot.facing = EAST
            elif direction == BACKWARD:
                self._canvas.move(self._head, 0, self._grid_size * 2)
                self._robot.facing = SOUTH
        elif facing == SOUTH:
            if direction == LEFT:
                self._canvas.move(self._head, self._grid_size, -self._grid_size)
                self._robot.facing = EAST
    ...
    def _move_robot(self, direction):
        ...
        f = switcher.get(self._robot.facing)

        if direction == BACKWARD:
            self._turn_head(self._robot.facing, RIGHT)
            self._turn_head(self._robot.facing, RIGHT)
        elif direction != FORWARD:
            self._turn_head(self._robot.facing, direction)
